...
const DEFAULT_PASSWORD = 'BSCS1-3'; // DEFAULT PASSWORD DITO
...
function checkAdminAccess() {
    ...
    database.ref('admin/password').once('value')
        .then(snapshot => {
            ...
            // Check against default OR database
            if(input === DEFAULT_PASSWORD || input === dbPassword) {
                sessionStorage.setItem('adminLoggedIn', 'true');
                showDashboard();
                loadDashboardStats();
                
                // Save default to DB if empty (first time)
                if(!dbPassword) {
                    database.ref('admin/password').set(DEFAULT_PASSWORD);
                }
            } else {
...
function loadAdminAnnouncements() {
    database.ref('announcements').orderByChild('timestamp').on('value', snapshot => {
        ...
        anns.forEach(ann => {
            ...
            div.innerHTML = `
                <div class="item-content">
                    <h4>MATH_INLINE_0{ann.priority}">${ann.priority}</span></h4>
...
function loadAdminComments() {
    ...
    query.once('value', snapshot => {
        ...
        comments.forEach(comment => {
            const div = document.createElement('div');
            div.className = `comment-admin-item ${comment.status}`;
            div.innerHTML = `
                <div class="comment-info">
                    <strong>${escapeHtml(comment.studentName)}</strong> 
                    <span class="year-badge">${escapeHtml(comment.yearLevel)}</span>
                    <span class="status-badge MATH_INLINE_2{comment.status}</span>
                    <br>
                    <small>ID: MATH_INLINE_3{formatDate(comment.timestamp)}</small>
